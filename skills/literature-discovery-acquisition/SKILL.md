...
1. **自动生成初稿推荐**：
   ...
   - 自动生成『推荐检索词矩阵初稿』与『四层级重点期刊初稿』；
2. **发起结构化 Grill-Me 提问（核心必选）**：
   向用户提出聚焦的边界确认问题（必须包含标有 `(Recommended)` 的推荐选项）：
   - **Q1 检索模式选择（核心第一必问）**：
     - `(Recommended) Deep Search`：完整 8 阶段流水线（多库联合、正交词全扩展、≥30篇多子代理并发初筛、引文双向追踪、PRISMA-S 16项机器质检与 OA 全文下载+Zotero沉淀）。适合：毕业论文开题、SCI综述、基金申报；
     - `Quick Search`：轻量 4 阶段流水线（高精检索式 + 四层级重点期刊精准过滤），极速获取 10–30 篇顶刊代表作与核心 OA，声明“非系统性探索”，严格控制上下文预算（<15 KB）。适合：组会速览研讨、前沿热点快速摸底、快速概念验证；
   - **Q2 硕博士学位论文专项需求（核心第二必问）**：中英博硕全覆盖 (默认，获取详尽实验参数) vs 仅中文博硕 vs 仅英文博硕 vs 不需要学位论文；
   - **Q3 语言与文献类型边界**：中英双语全覆盖（含研究论文与权威综述）vs 仅限英文同行评议；
   - **Q4 全文下载与建库选项**：自动批量下载开源 OA PDF 并生成 Zotero CSL-JSON (默认) vs 仅导出题录清单；
3. **协议快照锁定与分支流转**：用户确认后固化检索基线，并依据所选模式进入对应分支加载策略，写入检索日志。
