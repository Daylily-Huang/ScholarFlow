...
> **深度与模式正交**：研究目标不决定执行深度，**资源深度必须独立确认**。「开题 / 系统综述」
> 不是选择深度档的理由。无有效配置时展示三档并等待确认；**标准档是推荐值，不是可由任务措辞
> 静默推导的默认值**。下游技能沿用同一运行的已确认配置。

### 2.1 执行深度三档（`execution_depth`）

| 深度档 | 约算力预算 | 检索候选上限 | 概念扩展 / 滚雪球 | 定位 |
|---|:---:|:---:|:---:|---|
| `quick` 快速档 | ~5 分钟 | 20 | 0 / 0 | 快速精准探索：聚焦高精检索式，不做多轮引文追踪与饱和度计算，显式声明「非系统性完整检索」 |
| `standard` 标准档<br>**（推荐）** | ~20 分钟 | 50 | 1 / 1 | 均衡查全率与论证严密性：主检索 + 概念扩展 + 单轮引文追踪 |
| `deep` 深度档 | ~60 分钟 | 100 | 2 / 2 | 系统化全景发现：多轮扩展与引文追踪、饱和度追踪 |

### 2.2 执行方式

| 方式 | 核心定位 | 目标产出 | 执行特征 | 适用场景 |
|---|---|:---:|---|---|
| **交互工作流** | 人机协同把关 | 按深度档产出 | Stage 0 门禁 → 检索 → 初筛 → 获取；关键节点由学者裁决 | 学位论文、基金开题、系统评价、立项综述 |
| **Headless / Agent** | 机器纯数据执行管道 | 紧凑标准 JSON 流<br>(已初筛/去重/含OA直链) | 以显式 `--execution-depth` 静默执行全流程，直出机器消费级 JSON；**缺少或非法深度会返回结构化错误且不发起任何检索调用** | 供上层规划 Agent、论文写作智能体、自动化后台脚本消费 |

> **交付能力边界**（与 [`docs/CAPABILITY_STATUS.md`](../../docs/CAPABILITY_STATUS.md) 一致，冲突时以该表为准）：
> 去重实际为**两级**（归一化 DOI + 归一化标题），PMID / arXiv / 相似度匹配**未实现**；
> Stage 8B 浏览器兜底下载为 **`NOT_SUPPORTED`**，仅保留状态码与协议文档，不得作为正常交付路径承诺。
