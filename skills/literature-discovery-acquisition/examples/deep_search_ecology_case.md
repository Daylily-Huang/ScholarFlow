...
## Stage 4: 两级去重流水线（DOI + 归一化标题）

> 当前实现只有两级（见 `docs/CAPABILITY_STATUS.md` §1）：标准化 DOI 精确匹配与标准化标题精确匹配。
> PMID / arXiv 唯一标识匹配与"作者+年份+标题相似度"匹配**未实现**，本示例不演示这两级。

1. **第 1 级（标准化 DOI 精确匹配）**：合并 142 条重复记录；
2. **第 2 级（标准化标题精确匹配）**：合并 58 条标点/大小写差异导致的隐性重复；
3. **去重后独立候选文献总数**：**156 篇**（每篇完整保留 `source_databases` 来源列表）。
