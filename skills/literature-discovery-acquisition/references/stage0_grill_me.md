...
**自适应决策门禁的核心目标**：
1. **预设决策维度，动态生成问题**：基于 [grill_dimensions.md](./grill_dimensions.md) 定义的决策维度及共享 `EXECUTION_DEPTH`，结合任务语境与学科透镜，动态筛选 3~4 个最高影响度的问题发起提问；
2. **执行深度与研究目标正交独立**：执行前独立确认 `execution_depth`（quick/standard/deep）；研究目标（D1）不能替代深度选择。未指定且无有效配置时必须展示快速／标准／深度的工作范围与资源差异。未确认不得启动研究执行；
3. **每题必带推荐，推荐必给理由**：每个选项题必须包含 `(Recommended)` 选项，附带 1 句话的方法学Rationale与置信度标签（`[高置信度]` / `[中置信度]` / `[需权衡]`）；
4. **严格交互硬门禁 (STOP Rule)**：Agent 在输出 Stage 0 问题后，**必须立即终止当前回复，进入静默等待状态**，严禁在同一回复中自问自答或直接调用检索/下载工具；
...
| 层级 | 维度清单 | 处理策略 |
|---|---|---|
| **Tier 1: CRITICAL** | `EXECUTION_DEPTH` 执行深度与资源投入 (quick/standard/deep)<br>`D1` 研究目标与产出定位<br>`D2` 核心研究问题与因果命题<br>`D3` 目标实体与概念范畴<br>`D4` 纳入标准与边界<br>`D5` 排除标准与禁忌红线 | **首轮必须解决**；未在初始提示中明确者优先列入提问清单；未解决前严禁开工。`EXECUTION_DEPTH` 需显式确认，轮数耗尽不静默默认。 |
| **Tier 2: HIGH_IMPACT** | `D6` 概念拓展与近义词策略<br>`D7` 地理与空间范畴<br>`D8` 时间跨度与历史回溯<br>`D9` 文献类型与同行评审门槛 | 显著影响检索召回率与数据量；在不超过 3~4 题预算下按影响力降序填补提问槽位。 |
...
2. 从未决的 `CRITICAL` 维度中提取核心问题，并在 `HIGH_IMPACT` 维度中挑选高影响项，组合生成 **3~4 个** 聚焦的结构化问题；
3. **Agent 输出问题后立即停止输出（STOP），等待用户回复！**

**交互实施硬约束（针对结构化问答工具的题位限制）**：
- 当所用交互工具单轮题位上限为 4 时，**必须分轮提问**（每轮 ≤4 题，总预算仍为 3~4 题），不得为塞进单轮而压缩必要维度；
