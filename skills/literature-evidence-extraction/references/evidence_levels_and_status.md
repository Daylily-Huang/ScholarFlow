...
1. **第一层：Evidence Level（证据级别 E1–E4）** —— 衡量证据的直接性与来源属性；
2. **第二层：Evidence Status（证据状态）** —— 衡量证据对结论的支撑力度与矛盾状态。

---

## 二、第一层：四级证据级别定义与判定边界 (E1–E4)

```mermaid
flowchart TD
    Start[待提取字段] --> Q1{当前论文正文/表格/附录是否直接完整写出?}
    Q1 -- 是 --> E1[E1: EXPLICIT 原文明示]
    Q1 -- 否 --> Q2{是否由原文明示数据经简单数学运算直接得出?}
    Q2 -- 是 --> E2[E2: DERIVED 严密推导 必须附公式]
    Q2 -- 否 --> Q3{原文是否仅指引前人文献 e.g. following Smith 2012?}
    Q3 -- 是 --> E3[E3: REFERENCED 文献引述 本文值记为NR]
    Q3 -- 否 --> E4[E4: NR 未报告 严禁常识脑补]
```
