...
2. **预设决策维度，动态生成问题**：基于 [grill_dimensions.md](./grill_dimensions.md) 定义的合成决策维度及共享 `EXECUTION_DEPTH`，动态筛选 3~4 个最高影响度的问题发起提问；
3. **执行深度与任务类型正交独立**：执行前独立确认 `execution_depth`（quick/standard/deep）；综合任务类型（S1）不能替代深度选择。未指定且无有效配置时必须展示快速／标准／深度的工作范围与资源差异。未确认不得启动研究执行；
4. **每题必带推荐，推荐必给理由**：每个选项题必须包含 `(Recommended)` 选项，附带 1 句话的方法学依据与置信度标签；
5. **严格交互硬门禁 (STOP Rule)**：Agent 在输出 Stage 0 问题后，**必须立即终止当前回复，进入静默等待状态**，严禁在同一回复中自问自答或直接调用分析工具；
...
| 层级 | 维度清单 | 处理策略 |
|---|---|---|
| **Tier 1: CRITICAL** | `EXECUTION_DEPTH` 执行深度与资源投入 (quick/standard/deep)<br>`S1` 综合目的与产出定位<br>`S2` 核心争鸣命题与待检验假设<br>`S3` 证据库来源与纳入边界<br>`S4` 比较单元与聚合粒度 | **首轮必须解决**；未在初始提示中明确者优先列入提问清单；未解决前严禁开工。`EXECUTION_DEPTH` 需显式确认，轮数耗尽不静默默认。 |
...
3. **动态输出 3~4 个问题**：从未决的 S1、S2、S4、S5、S6、S7 中组织提问；
