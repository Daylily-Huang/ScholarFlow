...
### 四大原则
1. **预设决策维度，动态生成问题**：不预设死板的固定题目；按学科透镜与任务语境评估 9~14 个决策维度，动态筛选 3~4 个最高影响度的未决要素提问。
...
3. **严格交互硬门禁（STOP Rule）**：Agent 在输出 Stage 0 问题清单后，**必须立即终止当前回复，进入静默等待状态**，严禁在同一回复中自问自答或直接调用下游工具展开实质性工作。
   - **提问工具路由（强制）**：当宿主提供 `ask_user_question` 或任何等价的**结构化提问工具**时，Stage 0 的提问**必须通过该工具发起**，不得以纯文本清单代替（纯文本无结构化回执，且易在同一回复内被自答）。
   - **每轮题量上限**：**<= 4 题**。超出部分按优先级截断至下一轮，或对 `DEFAULTABLE` 维度静默应用默认值。
   - **STOP 的精确含义**：此处"终止回复"限定为**"不再调用除提问工具以外的任何工具"**——即本轮只允许调用提问工具，其余一切工具（检索、下载、抽取、读写产物等）一律不得在本轮调用。
...
| 指标 | 约束上限 | 设计逻辑 |
|---|---|---|
| **首轮问题数** | 3 ~ 4 题 | 避免认知过载，聚焦关键方法学分歧；上限 4 与 STOP Rule 的提问工具路由一致 |
...
stateDiagram-v2
    [*] --> STAGE0_NOT_STARTED
    STAGE0_NOT_STARTED --> STAGE0_UNRESOLVED: 分析任务 & 输出 3~4 题 (含 EXECUTION_DEPTH)
